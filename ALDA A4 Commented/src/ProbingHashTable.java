...
public abstract class ProbingHashTable<AnyType> {
	...
	/**
	 * Returns true if probing should continue
	 * Is true if currentPosition is not null and the element at current position
	 * is not equals to the element we are searching for (element is then found)
	 * @param currentPos	the currentPosition to check
	 * @param x				the item we are looking for
	 * @return				returns true if elements are the same
	 */
	...
	/**
	 * sets hashVal to the hashcode of x, then setting hashVal to modulo of array length.
	 * if the hashVal is then lesser than 0, then add the length of the array
	 *
	 * @param x the value we want to hash
	 * @return hashVal,
	 */
